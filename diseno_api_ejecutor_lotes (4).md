...
**Operaciones disponibles según el estado del servicio:**

Cuando `gesprog` está **corriendo** se aceptan todas las operaciones:
`guardar`, `leer`, `actualizar`, `borrar`, `suspender`, `terminar`

Cuando `gesprog` está **suspendido** solo se aceptan:
`leer`, `reasumir`, `terminar`

Cualquier otra operación enviada mientras el servicio está suspendido retorna el error `SERVICIO_SUSPENDIDO`.
...
Cuando `gesfich` está **corriendo** se aceptan todas las operaciones:
`crear`, `leer`, `actualizar`, `borrar`, `suspender`, `terminar`

Cuando `gesfich` está **suspendido** solo se aceptan:
`leer`, `reasumir`, `terminar`
...
Cuando el ejecutor está **corriendo** se aceptan todas las operaciones:
`ejecutar`, `estado`, `matar`, `suspender`, `parar`

Cuando el ejecutor está **suspendido** se aceptan:
`estado`, `matar`, `reasumir`, `parar`

Cuando el ejecutor está en estado **parar** se aceptan:
`estado`, `matar`

Cuando el ejecutor está en estado **parar**, no acepta nuevas ejecuciones pero los lotes ya en curso continúan corriendo hasta terminar. Una vez que no quedan procesos activos, el ejecutor pasa automáticamente a `terminado`.
